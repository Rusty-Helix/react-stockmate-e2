...
import { strategyStatType } from "../utils/Types";
...
import { currentStrategyType, strategyStatsType } from "../utils/Types";

export default function Info({
  data,
}: {
  data: currentStrategyType | undefined;
}) {
  ...
  const createStatsArray = (types: string[], statType: string) => {
    ...
    types.forEach((type: string) => {
      // @ts-ignore
      strategyTypes[type][statType].forEach((stat: string) => {
  ...
  return (
    <>
      <div className="details">
        ...
        <button onClick={() => dispatch(setStrategyTab(strategyTabs.evolution))}>
          See next evolution
        </button>
      </div>
      <div className="stats">
        <ul>
          {data?.stats.map((stat: strategyStatsType) => {
            return (
              <li key={stat.name}>
      ...
      <div className="battle-stats">
        ...
        <button
          onClick={() => dispatch(addStrategyToList(data!))}
          className="add-strategy"
        >
          Add Pokemon
        </button>
      </div>
